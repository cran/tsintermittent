...
# Nikolaos Kourentzes, 2014 <nikolaos@kourentzes.com>
  ...
  if (is.null(w)){
    wopt <- crost.opt(data,type,cost,nop,init,init.opt)
    w <- wopt$w
    init <- wopt$init
  }
...
# Optimisation function for Croston and variants
  ...
  if (init.opt == TRUE){
    w <- c(rep(0.05,nop),init[1],init[2])
    lbound <- c(rep(0,nop),0,1)
    ubound <- c(rep(1,nop),max(data),max(x))
    wopt <- optim(par=w,crost.cost,method="Nelder-Mead",data=data,cost=cost,
                  type=type,nop=nop,init=init,init.opt=init.opt,lbound=lbound,ubound=ubound,
...
#-------------------------------------------------
crost.cost <- function(w,data,cost,type,nop,init,init.opt,lbound,ubound){
# Cost functions for Croston and variants  
  
  if (init.opt==TRUE){
    frc.in <- crost(data=data,w=w[1:nop],h=0,init=w[(nop+1):(nop+2)],
                    type=type,opt.on=TRUE)$frc.in
  } else {
    frc.in <- crost(data=data,w=w[1:nop],h=0,init=init,type=type,opt.on=TRUE)$frc.in
  }
